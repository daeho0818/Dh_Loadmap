...
const famOf = new Map(species.map((s) => [s.id, Number(s.evolution_chain_id)]));
...
for (const p of pokemon) {
  ...
  const fam = famOf.get(p.species_id) || 0;

  if (p.is_default === '1') {
    mons.push([Number(p.species_id), gen, tt[0], tt[1] || 0, nm.ko, nm.en, Number(p.id), main, fam]);
    continue;
  }
  ...
  mons.push([Number(p.species_id), formGen, tt[0], tt[1] || 0, name2[0], name2[1], Number(p.id), main, fam]);
}
mons.sort((a, b) => a[0] - b[0]);

const out = `/* generated by tools/make-data.mjs (${new Date().toLocaleDateString('sv-SE')}) — do not edit by hand.
   source: PokeAPI CSV. mons: [dexNo, gen, type1, type2(단일이면 0), koName, enName, pokeId, main, fam]
   (pokeId는 스프라이트 URL용 — 폼은 dexNo와 다름 · main=1: 최종 진화/전설 · fam: 진화 라인 id) */
