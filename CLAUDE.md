...
| 004 | 수면 실험실 (Sleep Lab) | 밤 기록(태그+아침 회고) × 삼성헬스 가져오기 → 분석·솔루션. 소유자 발안 | **설계 중** |
| 005+ | 알찬 지도 / 나 사용 설명서 | 시간×욕구 측정 / 전체 데이터 종합 | 백로그 |

#004 확정 사항: 기록은 **밤에** (그날 변수 태그 + 오늘 아침 기상 회고). 솔루션은
`data/sleep-solutions.json`(규칙 DB, 커밋됨)에서 트리거 평가. 개인 수면 데이터
(`data/samsung-health/`, `data/sleep*.json`)는 gitignore — 절대 커밋 금지.
