...
| 004 | 수면 실험실 (Sleep Lab) | 밤 기록(태그+아침 회고) × 삼성헬스 가져오기 → 분석·솔루션. 소유자 발안 | 구현됨 (실사용 시작) |
| 005+ | 알찬 지도 / 나 사용 설명서 | 시간×욕구 측정 / 전체 데이터 종합 | 백로그 |

#004 확정 사항: 기록은 **밤에** (그날 변수 태그 + 오늘 아침 기상 회고). 솔루션 규칙 DB는
`experiments/004-sleep-lab/solutions.json` (페이지가 fetch — file:// 직접 열기로는 처방이
안 뜨고 HTTP 접속 필요). 개인 수면 데이터(`data/samsung-health/`, `data/sleep-log*.json`,
`data/evidence*.json`)는 gitignore — 절대 커밋 금지.
분석 스크립트: 세션 스크래치패드 `analyze-sleep.js` (500일 밤 분석 결과는 아래 요약 참고).
소유자 데이터 핵심: 수면량 7h14m(양호) / 문제는 입면 지연(최근 중앙값 27분)과
취침 요동(SD 82분) / 취침 빠른 달 = 점수·수면량 동반 상승 (개인 상관 확인됨).
